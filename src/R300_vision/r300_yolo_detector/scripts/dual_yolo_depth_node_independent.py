...
# 两个模型均为独立训练的子模型，YOLO输出的是各自模型内部的局部类别ID。
# 发布前分别映射到统一的13类全局编号；两个模型之间不做覆盖、互斥或跨模型NMS。
...
class DualYoloDepthNode:
    ...
    模型1：独立6类模型，负责：
        vehicle、smoke、trench、person、park、crater

    模型2：独立7类模型，负责：
        tire、barrel、puddle、rockfall、chevro_left、chevro_right、sandbag

    两个模型独立推理、独立应用类别阈值，结果仅做全局类别ID映射后拼接。
    不执行模型间覆盖、互斥或跨模型NMS。
    ...
    def __init__(self) -> None:
        ...
        # 基础阈值用于未在类别阈值字典中配置的类别，同时参与predict候选阈值计算。
        ...
        # predict()先用各模型所有最终阈值中的最小值保留候选框，
        # 解析结果时再按类别应用最终阈值，避免高阈值类别影响低阈值类别召回。
    ...
    def _validate_parameters(self) -> None:
        ...
        expected_model1_names = {
            GLOBAL_CLASS_ID_TO_NAME[global_id]
            for global_id in MODEL1_LOCAL_TO_GLOBAL.values()
        }
        expected_model2_names = {
            GLOBAL_CLASS_ID_TO_NAME[global_id]
            for global_id in MODEL2_LOCAL_TO_GLOBAL.values()
        }
    ...
    def _validate_class_thresholds(
        ...
        if unknown_names:
            raise ValueError(
                f"参数 ~{parameter_name} 包含不属于该模型的类别："
    ...
    def _validate_model_class_mappings(self) -> None:
        checks = (
            ("模型1", self.model1, MODEL1_LOCAL_TO_GLOBAL),
            ("模型2", self.model2, MODEL2_LOCAL_TO_GLOBAL),
        )

        for model_label, model, mapping in checks:
            expected_names = {
                local_id: GLOBAL_CLASS_ID_TO_NAME[global_id]
                for local_id, global_id in mapping.items()
            }
            actual_names = self._normalized_model_names(model)

            if actual_names != expected_names:
                raise RuntimeError(
                    f"{model_label}类别顺序与代码映射不一致。\n"
                    f"期望：{expected_names}\n"
                    f"实际：{actual_names}\n"
                    "请检查权重文件或修改LOCAL_TO_GLOBAL映射。"
                )

            rospy.loginfo(
                "%s class mapping validated: %s",
                model_label,
                str(expected_names),
            )
    ...
    def parse_model_result(
        ...
        YOLO输出局部类别ID；本函数映射到13类全局ID，并按类别应用最终阈值。
        两个模型之间不做任何覆盖或抑制。
        """
        ...
        for xyxy, class_value, confidence_value in zip(
            ...
            if local_id not in local_to_global:
                rospy.logwarn_throttle(
                    5.0,
                    "%s returned unknown local class id=%d",
                    source_model,
                    local_id,
                )
                continue
    ...
    def inference_timer_callback(self, _event) -> None:
        ...
        # 两个模型完全独立：不做覆盖、互斥或跨模型NMS，仅拼接后统一发布。
        merged_candidates = model1_candidates + model2_candidates
