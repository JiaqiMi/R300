...
function drawScan() {
  ...
  if ($("showScanRaw").checked) drawLaser(ctx, scanData, origin, scale, "#ff3b1f", 1.5);
  if ($("showScanVision").checked) drawLaser(ctx, visionScanData, origin, scale, "#f6d33b", 2.8);
  if ($("showScanActive").checked) drawLaser(ctx, activeVisionScanData, origin, scale, "#a8f02a", 3.0);
  drawRobotArrow(ctx, origin.x, origin.y, -Math.PI/2, 24, "#a8f02a", "#f2ffd2");
  ctx.restore();
